...
const clientEnvSchema = z.object({
  NEXT_PUBLIC_APP_NAME: z.string().min(1).default('Barberly Admin'),
  NEXT_PUBLIC_BRAND_NAME: z.string().min(1).default('Barberly'),
  NEXT_PUBLIC_SITE_URL: z.string().url().optional(),
  NEXT_PUBLIC_OTP_DEV_BYPASS_CODE: z.string().length(6).optional(),
...
const parsedClient = clientEnvSchema.safeParse({
  ...
  NEXT_PUBLIC_OTP_DEV_BYPASS_CODE:
    process.env.NEXT_PUBLIC_OTP_DEV_BYPASS_CODE,
...
export const env = {
  ...
  OTP_DEV_BYPASS_CODE:
    parsedClient.success && parsedClient.data.NEXT_PUBLIC_OTP_DEV_BYPASS_CODE
      ? parsedClient.data.NEXT_PUBLIC_OTP_DEV_BYPASS_CODE
