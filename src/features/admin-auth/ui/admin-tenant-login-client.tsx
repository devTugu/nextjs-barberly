...
interface AdminTenantLoginClientProps {
  tenantName: string;
  logoUrl?: string | null;
  brandColor?: string | null;
}

export function AdminTenantLoginClient({
  tenantName,
  logoUrl,
  brandColor,
}: AdminTenantLoginClientProps) {
  const [step, setStep] = useState<SignInStep>('credentials');
  const locale = useLocale() as Locale;
  const t = useTranslations('adminAuth');
  const stepHeading = getStepHeadings(locale)[step];
  const heading =
    step === 'credentials'
      ? {
          title: t('signInTitle', { tenantName }),
          subtitle: t('signInSubtitle'),
        }
      : stepHeading;
  const brandStyle: CSSProperties | undefined = brandColor
    ? ({ '--brand-primary': brandColor } as CSSProperties)
    : undefined;
  ...
  return (
    <div className="grid min-h-svh lg:grid-cols-2" style={brandStyle}>
      <div className="flex flex-col gap-4 p-6 md:p-10">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 font-medium">
            ...
            <BrandLogo name={tenantName} logoUrl={logoUrl} showName />
          </div>
          <LocaleSwitcher />
        </div>
